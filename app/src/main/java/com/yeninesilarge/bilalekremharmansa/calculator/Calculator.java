package com.yeninesilarge.bilalekremharmansa.calculator;
...
public class Calculator implements Parcelable{
    ...
    public String evaluate() {
        //private olarak BigDecimal nesnesi döndüren evaluate methoduna postfix stringini geçerek geriye sonucu string olarak alıyor.
           ...
           try {
               ...
               throw new SyntaxErrorException("Syntax hatalı!!");
           }
    ...
    private BigDecimal divide(BigDecimal numOne, BigDecimal numTwo) throws SyntaxErrorException {
        if (numTwo.intValue() == 0) {
            logger.debug("Some log message. Details: {}", "sıfıra bölünemez".toString());
    ...
    public boolean appendOperationLine(char c) {

        //eğer bi karakter eklenirse true olacak
        boolean characterAppended = false;
        ...
        if(currentResult.equals("0") && !operationLine.toString().equals("") && characterAppended){
            currentResult = "";
        }
    ...
    public void writeToParcel(Parcel dest, int flags) {
        dest.writeSerializable(this.operationLine);
        dest.writeString(this.currentResult);
        dest.writeByte(this.resultFlag ? (byte) 1 : (byte) 0);
        dest.writeByte(this.negativeFlag ? (byte) 1 : (byte) 0);
        dest.writeInt(this.openBracketCounter);
    }

    protected Calculator(Parcel in) {
        this.operationLine = (StringBuilder) in.readSerializable();
        this.currentResult = in.readString();
        this.resultFlag = in.readByte() != 0;
        this.negativeFlag = in.readByte() != 0;
        this.openBracketCounter = in.readInt();
    }
